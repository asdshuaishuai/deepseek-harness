...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    ...
    expect(view.getByText('Into the Intelligence Leap')).toBeTruthy()
    ...
    // This fork draws no whale: the brand-mark slot stays an extension point
    // with no fallback, so an unoccupied hero is headline text alone.
    expect(renderSlot.mock.calls[0]?.[2]?.fallback ?? null).toBeNull()
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: keeps sidebar controls accessible while hiding conversation chrome', () => {
    ...
    expect(b.view.getByText('探索智能跃迁之境')).toBeTruthy()
  ...
  it('keeps a rejected first prompt engaging instead of returning to the Hero', () => {
    ...
    expect(b.view.queryByText('探索智能跃迁之境')).toBeNull()
  })
