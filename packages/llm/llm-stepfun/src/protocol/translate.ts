/**
 * Translate StepFun SSE payloads into the harness `StreamChunk` protocol with
 * one stateful block per content, reasoning, or tool-call index. An empty
 ...
 *
 * @module dsh-llm-stepfun/translate
 */
...
/**
 * Map wire usage fields. Endpoints that fold cache hits into
 * `prompt_tokens` report the hit count alongside; the harness TokenUsage
 * convention is disjoint counts, so a reported hit count is subtracted out of
 * `inputTokens`.
 ...
 * OpenAI-compatible gateways fill in — means "no update", never "clear".
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      // Reasoning first: automatic thinking interleaves it before text. The
