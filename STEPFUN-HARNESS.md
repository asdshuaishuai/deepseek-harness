...
### Hidden from the composition (StepFun-only, for now)

"剔除" here means **unmounted, not deleted**: the external-provider packages stay in the repository so upstream merges stay cheap; no shipped profile mounts them. `dsh-base` no longer mounts: `llm-deepseek`, `llm-pi-ai` (the multi-provider OpenAI/Anthropic/Google/… twin), `deepseek-llm-api-extensions`, `session-log-deepseek`, `plugin-package-inventory-deepseek`, and `web-search-deepseek`; the standalone `sdk-minimal` profile swaps the same DeepSeek rows for `llm-stepfun` plus the StepSearch MCP. Web search reaches the model through two StepFun routes: the StepSearch MCP above (`mcp__stepfun-search__web_search`/`web_fetch`, billed against the Step Plan subscription's monthly credit — 0.04 CNY per `web_search`, `web_fetch` unbilled) and the seam-level [`dsh-web-search-stepfun`](packages/web/web-search-stepfun/README.md) provider (`web_search` over the open-platform `POST /v1/search`, the route the platform recommends for `step-5-preview`, billed per the open-platform web-search pricing); `web_fetch` stays on the provider-neutral HTTP fetcher. OTel telemetry is switched off by default (its exporter endpoint is an external service this fork does not ship).

## Known edges

- The URL guard judges hostnames and IP literals only — it does not resolve DNS, so a name that resolves into private space passes. This matches the guard's contract (operator configuration is trusted; untrusted-input callers resolve and re-check), documented in `dsh-url-guard`.
- The channel-resolution shape (channel enum + endpoint map + env fallback) is deliberately triplicated across `llm-stepfun`, `stepfun-realtime`, and `mcp-stepfun-search` rather than shared through a fourth package; consolidating it is deferred until a fourth consumer appears.
- The REAL composition suite for `llm-stepfun` covers boot, registration, settings hot-swap, credentials, and the fail-loud surfaces without network I/O — the fork's URL guard rejects loopback endpoints, so wire-protocol behavior is owned by the unit protocol suites instead of a local mock server.
